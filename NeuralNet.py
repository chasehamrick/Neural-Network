...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):                                       
        ...
        """
        """YOUR CODE"""
        inActs2 = [1] + list(inActs)
        weightedSum = self.getWeightedSum(inActs2)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        inActs2 = [1] + list(inActs)
        return self.sigmoidDeriv(self.getWeightedSum(inActs2))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in xrange(len(self.weights)):
            modification = alphaDelta * inActs2[i]
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                answer[index + 1].append(perceptron.sigmoidActivation(input))
            index += 1
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = gPrime * sum([nextLayer[i].weights[neuronNum + 1] * deltas[0][i] for i in xrange(len(nextLayer))])
